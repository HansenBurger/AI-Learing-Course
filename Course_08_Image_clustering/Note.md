...
首先随机两个质心($P1$, $P2$)，计算剩余点到他们的欧式距离，可以得到:
...
其中 $n$ 为组中样本的数量，$P1$ 因为只有一个，所以质心不改变，$P2$ 变为 $PN=(6.2, 5.6)$，再次计算除质心外的点($P2$-$P6$)到质心的欧式距离，可以得到:
...
依据之前的均值方法，再次选出新的质心 $PN1=(1.33, 1)$，$PN2=(9, 8.33)$，重复上述步骤，得到:
